...
if __name__=='__main__':
  ...
  for idy, seach_date in enumerate(ltdates):
    ...
    for ifrst_hour in ltforecast_hours:
      ...
      #输出文件绝对路径
      if args.out_path.strip()=="" or args.out_path==None:
      ...
      ltcycle.append([sIn_Abs_Path,sOut_Abs_Path])
      print(sIn_Abs_Path)
      print(sOut_Abs_Path)
      # #获取原始数据大小
      ...
      # #数据提取
      # cls_s2wd.dS2_Extract(sIn_Abs_Path,sOut_Abs_Path,ltArea,args.delete)
      # #获取新数据大小
      # if os.path.exists(sOut_Abs_Path): 
    ...
    # pool_result = pool.map(cls_s2wd.dmulti_grib_modify, ltcycle)
    # pool.close()
    # pool.join()
