...
if __name__=='__main__':
  ...
  if os.path.exists(spara_abs_path):
    with open(spara_abs_path, "r") as fh:
      #纬度
  ...
  #切割格点区域
  ltArea = [float(x) for x in lat_vars[0:2]+lon_vars[0:2]]
  ...
  for idy, seach_date in enumerate(ltdates):
    ...
    for ifrst_hour in ltforecast_hours:
      ...
      sIn_mian_path = cls_model.GetDisk(seach_date)
      #输入文件绝对路径
      sIn_Abs_Path = os.path.join(sIn_mian_path, sModel_name+"_"+sRegion, sBegin_Date_BJ[0:4],
                                  sBegin_Date_BJ[0:8]+"_"+args.sbegin_hour, sIn_file_name)
      #输出文件绝对路径
      sOut_Sub_Path = os.path.join(cls_gp.sResult_Main_Path, ltwork[-2], sModel_name+"_"+sRegion, sBegin_Date_BJ[0:4],
      ...
      if ifrst_hour==0:
        ...
        #输入文件绝对路径
        sIn_Abs_Path1 = os.path.join(sIn_mian_path, sModel_name+"_"+sRegion, sBegin_Date_BJ[0:4],
