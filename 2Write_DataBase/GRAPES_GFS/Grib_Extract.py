...
if __name__=='__main__':
  ...
  cls_s2wd  = M2WD.Class_GRAPES_GFS_2Write_DataBase(0)
  #获取参数
  ...
  if args.grid_name is None:
    ...
    ltArea = [latlon_info.lat_start-0.001, latlon_info.lat_end+0.001, latlon_info.lon_start, latlon_info.lon_end] #(南北西东)
  #GRAPES_MESO是0.1分辨率,精度不够会产生剪裁减小0.1的情况。
  ...
  for idy, seach_date in enumerate(ltdates):
    ...
    for ifrst_hour in ltforecast_hours:
      ...
      #输入文件名                                        
      sIn_file_name = "Z_NAFP_C_BABJ_"+sBegin_Date_UTC+"0000_P_NWPC-GRAPES-GFS-HNEHE-"+sfrst_hour+"00.grib2"
      #输入文件绝对路径
